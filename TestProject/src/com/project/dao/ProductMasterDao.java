...
public class ProductMasterDao {
    ...
    public boolean saveProductMaster(ProductMaster productMaster){
        try {
            Connection con = Database.getConnection();
            PreparedStatement ps = con
                    .prepareStatement("UPDATE ProductMaster SET ProductName = ? ,Cost = ?,ProductDescription = ? WHERE id = ?");
  ...
  private JSONArray getProductMasterDetails(final int productMasterId){
        ...
        if (productMasterId == 0) throw new NullPointerException("ProductMaster Id can't be null");
        try {
            Connection con = Database.getConnection();
